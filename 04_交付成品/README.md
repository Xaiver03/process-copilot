...
- `序安_磷煤化工异常早期预警平台_黑客松路演_v14_ZH_REVIEW.pptx`：10 页、16:9、从空白画布重建的可编辑路演；统一使用 v04“异常边界截获”标志，加入新生成工业主视觉和 Playwright 公网回放实拍。
- `序安_磷煤化工异常早期预警平台_黑客松路演_v14_ZH_REVIEW.pdf`：与 v14 PPTX 对应的 10 页评审 PDF，已逐页渲染、字体嵌入与溢出检查。
- 配套逐字稿：`../docs/submission/序安_磷煤化工异常早期预警平台_黑客松路演逐字稿_v14_REVIEW.md`，与 10 页演讲者备注逐字一致。
- v14 可重建源与 QA：`../90_构建与分析缓存/序安磷煤化工异常早期预警平台_黑客松路演_v14/`。
...
当前评审版：`序安_Process_Sentinel_连续化工过程偏移副驾驶_产品手册_v08_ZH_REVIEW.pdf`（25 页，16:9；覆盖本地与公网各 11/11、23 个关键状态、92 张首屏/全页证据、SSOS AI 配置路径复用、`deepseek-chat` 真实在线回答、工业模型证据、调用同链审计、密钥不回显、人工编辑与 5 道影子门禁）。

当前 v08 对应中央 QA 标签 `序安产品手册_SSOS复用与真实AI_v01_2026-08-29`，25 页均已渲染检查；手册使用最新公网 Playwright 首屏，所有对应全页长图继续保存在验收证据目录并记录哈希。
