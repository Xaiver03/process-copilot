...
应用基线提交：`f9519001049fe009dfe160bbacc9b34d612b067a`
...
两套环境均为 `10/10 passed`，无跳过、无重试、无 flaky。每套环境保存 22 个关键状态，每个状态同时保存“当前视口”和“完整页面”，即每套 44 张、合计 88 张 Playwright 截图。其中 UJ-09、UJ-10 专门验证 AI 的工业推理、证据引用、人工追问、调用审计、运行状态、降级透明度、密钥与配置安全边界。

| 环境 | 旅程 | 通过 | 失败 | 跳过 | 首屏截图 | 全页截图 |
|---|---:|---:|---:|---:|---:|---:|
| 本地当前代码 | 10 | 10 | 0 | 0 | 22 | 22 |
| 公网部署 | 10 | 10 | 0 | 0 | 22 | 22 |
...
| 中控操作员 | 在线追问、升级上报、查看自己的审计记录 | 不能确认或驳回；不能进入管理后台 | UJ-03、UJ-06、UJ-09 |
...
用户动作：系统管理员检查 AI 运行概览和在线 AI 配置；同时直接核对状态与配置 API；尝试更新配置和发起连接测试。
...
- 在线工业模型负责连续偏移检测和候选更新；语言模型未配置时返回明确标记的模板/降级答案，不伪装成在线大模型。
- 当前 Demo 不向 APC、DCS、PLC 或 SIS 写回。页面只展示受控写回草案；生产版应在独立权限、约束、联锁、双人确认、受控网关、读回和回滚机制完成后再开放。
...
- 本地与公网 Playwright 均 10/10；
- 88 张截图及两个哈希清单存在且通过校验；
