...
应用基线提交：`012fa867f163fe1aad0e640c35751cd3d75e7d12`
...
两套环境均为 `11/11 passed`，无跳过、无重试、无 flaky。每套环境保存 23 个关键状态，每个状态同时保存“当前视口”和“完整页面”，即每套 46 张、合计 92 张 Playwright 截图。其中 UJ-09 至 UJ-11 专门验证 AI 的工业推理、证据引用、人工追问、调用审计、运行状态、降级透明度、密钥与配置安全边界，以及 AI Trace 绑定人工编辑动作后的持久化影子门禁。

| 环境 | 旅程 | 通过 | 失败 | 跳过 | 首屏截图 | 全页截图 |
|---|---:|---:|---:|---:|---:|---:|
| 本地当前代码 | 11 | 11 | 0 | 0 | 23 | 23 |
| 公网部署 | 11 | 11 | 0 | 0 | 23 | 23 |
...
用户动作：以系统管理员登录，分别使用 1440×900、1024×768、390×844 视口访问演示、回放、事件和后台页面。

通过标准：每个视口下 `document.scrollWidth` 与 `body.scrollWidth` 均不超过视口宽度 1 像素；主内容可见；事件页分别保留完整截图。

证据状态：`UJ08-桌面1440_事件页`、`UJ08-平板1024_事件页`、`UJ08-手机390_事件页`。
...
通过标准：推理模式为在线，Worker 和工业模型就绪，工业模型版本可识别；语言模型状态只允许就绪、降级、离线或未知，非就绪时必须给出原因；配置响应只返回 `apiKeyConfigured`，不得含 `apiKey` 或疑似密钥；密码输入框加载后为空；公开环境的配置写入与连接测试均返回 `403 admin_ai_read_only`。
...
- 在线工业模型负责连续偏移检测和候选更新；当前公网 `deepseek-chat` 已产生真实 `llm_enhanced` 回答，若单次外部调用不可用则返回明确标记的模板/降级答案，不伪装成功。
