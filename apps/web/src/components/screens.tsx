...
import { eventSeverityPresentation, eventStateLabel, formatFaultCandidate, formatPredictionRisk, formatScenarioPresentation, localizeIndustrialCopy } from "@/lib/presentation";
...
export function OverviewScreen() {
  ...
  const prediction = demoEvent.prediction;
  const overviewCandidate = prediction
...
export function ReplayScreen() {
  ...
  useEffect(() => {
    if (!journey || (journey.mode !== "static-demo" && journey.data.run.inferenceMode !== "template") || journey.data.run.state !== "playing" || replayCompleted) return;
  ...
  async function startReplay() {
    ...
    try {
      if (journey?.data.run.state === "paused") {
        const result = await controlRunWithFallback(
          journey.data.run.id,
          { action: "play", speed: normalizeReplaySpeed(journey.data.run.speed) },
          { ...journey.data.run, currentSample: displaySample },
        );
        setJourney({ ...journey, data: { ...journey.data, run: result.data }, mode: result.mode, notice: result.notice });
      } else {
        const result = selectedScenario?.recommendedInferenceMode === "template"
          ? await startScenarioWithFallback(selectedId, 10)
          : await startOnlineScenarioWithFallback(selectedId, 10);
        setJourney(result);
  ...
  return (
    <div className="page-stack replay-page">
      <PageHeader kicker="过程回放" title={selectedScenario?.domain === "wastewater" ? "污水出水风险回放" : "52 路连续过程数据回放"} summary={selectedScenario?.domain === "wastewater" ? "回放公开过程变量，观察 AI 如何在下一化验周期之前给出风险、边界和核查顺序。" : "热力图先定位变量组，再用事件研判页查看三条对齐证据。"} />
...
export function EventsScreen({ runId }: { runId?: string }) {
  return (
    <div className="page-stack">
      <PageHeader kicker="风险事件" title="事件队列" summary="按严重度和发生时间筛选，污水预测与连续过程偏移都使用同一条人工确认和审计链路。" />
  ...
  return (
    <div className="page-stack event-detail-page">
      <ResourceBoundary {...resource}>{(result) => {
        ...
        const predictionRisk = event.prediction ? formatPredictionRisk(event.prediction.riskLevel) : null;
        return (
          <>
            <PageHeader kicker="AI 事件研判" title={event.prediction ? "污水出水风险预判" : (topCandidate?.label ?? `偏移事件 ${event.id}`)} summary={event.prediction ? "AI 已完成出水风险预测、历史边界和不确定区间整理，当前等待人工确认。" : `AI 已完成偏移发现、故障候选排序和变量证据对齐，当前等待${eventStateLabel[event.state] === "待研判" ? "人工确认" : "查看人工结论"}。`} />
            <ModeNotice mode={result.mode} notice={result.notice} />
            <ol className="ai-stepper" aria-label="AI 研判主链路">
              ...
              <li>
                <a className="ai-step-link" href="#step-02-conclusion" aria-current={activeAiStep === "step-02-conclusion" ? "step" : undefined} onClick={(event) => { event.preventDefault(); goToStep("step-02-conclusion"); }}>
                  <Brain aria-hidden="true" /><span>02</span><strong>判断</strong><small>{event.prediction ? "计算未来风险" : "排序故障候选"}</small>
                </a>
              </li>
              <li>
            ...
            <div className="ai-workbench">
              <section className="ai-panel ai-detection" data-ai-step="1" aria-labelledby="ai-detection-title" id="step-01-detection" tabIndex={-1}>
                <div className="ai-section-header"><div><span className="kicker">步骤 01 · AI 发现</span><h2 id="ai-detection-title">{event.prediction ? "风险何时被看见" : "偏移何时被看见"}</h2></div></div>
                <p className="ai-sample-window">{event.prediction ? `预测输入样本 ${event.detectionSample} → ${event.prediction.horizonLabel}` : `检测样本 ${event.detectionSample} → 诊断样本 ${event.diagnosisSample}`}</p>
                <dl className="ai-stat-grid">
                  <div><dt>{event.prediction ? "风险分数" : "首次异常分数"}</dt><dd>{event.anomalyScore.toFixed(2)}</dd></div>
                  <div><dt>{event.prediction ? "预测中心值" : "诊断异常分数"}</dt><dd>{event.prediction ? event.prediction.predictedValue.toFixed(2) : event.diagnosisAnomalyScore.toFixed(2)}</dd></div>
                  <div><dt>{event.prediction ? "预测窗口" : "候选刷新延迟"}</dt><dd>{event.prediction ? event.prediction.horizonSamples : event.diagnosisDelaySamples}<small> 样本</small></dd></div>
                  <div><dt>{event.prediction ? "人工复核" : "告警锁存"}</dt><dd>{event.prediction ? "待确认" : event.anomalyLatched ? "已锁定" : "未锁定"}</dd></div>
                </dl>
                <p className="ai-method-note">{event.prediction ? "随机森林软测量使用当前 7 项在线过程变量预测下一条公开记录的 DQO-S。历史 P95 仅作演示边界，不是法规限值。" : "持续性和滞回规则先过滤瞬时噪声，再由 PCA T² / SPE 判断过程是否偏离正常空间。"}</p>
              </section>
              ...
              <section className="ai-panel ai-conclusion" data-ai-step="2" aria-labelledby="ai-conclusion-title" id="step-02-conclusion" tabIndex={-1}>
                ...
                <p className="ai-thesis">{event.prediction ? <>下一化验周期的<strong>{event.prediction.targetName}</strong>预测值为 <strong>{event.prediction.predictedValue.toFixed(1)} {event.prediction.unit}</strong>，当前风险等级为<strong>{predictionRisk?.label}</strong>。</> : <>当前最可能是<strong>{topCandidate?.label ?? "候选尚未收敛"}</strong>，AI 将它排在首位的依据来自同步时间窗内的三项高贡献变量。</>}</p>
                <p className="ai-value-prop">{event.prediction ? "提前预测出水风险，并给出边界与不确定区间" : "不只报异常，还给出故障假设与变量证据"}</p>
                <ol className="ai-candidate-list" aria-label={event.prediction ? "AI 核查优先级" : "AI 故障候选排序"}>
                  {event.candidates.map((candidate, index) => {
                    const display = event.prediction
                      ? { code: `核查 ${String(index + 1).padStart(2, "0")}`, label: candidate.label, probability: `${Math.round(candidate.probability * 100)}%` }
                      : formatFaultCandidate(candidate);
              ...
              <section className="ai-panel ai-explanation" data-ai-step="3" aria-labelledby="ai-explanation-title" id="step-03-explanation" tabIndex={-1}>
                ...
                <p className="ai-explanation-copy">{event.prediction ? <>AI 将 {event.evidence.map((item) => `${item.variableId} ${localizeIndustrialCopy(item.variableName)}`).join("、")} 按同一文件行序对齐，用于排定人工核查顺序；它们不单独证明根因或法规超限。</> : <>AI 把变量变化放到同一个时间轴上比较：{event.evidence.map((item) => `${item.variableId} ${localizeIndustrialCopy(item.variableName)}`).join("、")}共同指向当前故障假设。</>}</p>
                <EvidenceTrendChart evidence={event.evidence} onsetSample={event.detectionSample} />
                {event.prediction ? <section className="prediction-evidence" aria-label="出水风险预测证据">
                  <div className="ai-section-header"><div><span className="kicker">出水风险预测</span><h3>下一化验周期预测值</h3></div><StatusTag state={predictionRisk?.state ?? "warning"} label={predictionRisk?.label ?? "未知 · 待人工确认"} /></div>
                  <dl className="prediction-grid"><div><dt>预测目标</dt><dd>{event.prediction.targetName}</dd></div><div><dt>预测值</dt><dd>{event.prediction.predictedValue.toFixed(1)} <small>{event.prediction.unit}</small></dd></div><div><dt>历史高位边界</dt><dd>{event.prediction.historicalHighBoundary.toFixed(1)} <small>{event.prediction.unit}</small></dd></div><div><dt>不确定区间</dt><dd>{event.prediction.lowerBound.toFixed(1)}–{event.prediction.upperBound.toFixed(1)} <small>{event.prediction.unit}</small></dd></div></dl>
                  <p>{event.prediction.horizonLabel} · 误差参考 MAE {event.prediction.uncertaintyMae.toFixed(1)} {event.prediction.unit} · {event.prediction.boundaryBasis}</p>
                </section> : <div className="ai-evidence-detail"><EvidencePanel evidence={event.evidence} /><ContributionChart evidence={event.evidence} /></div>}
              </section>
      ...
      <ResourceBoundary {...resource}>{(result) => (
        <>
          ...
          <section className="system-grid">
            ...
            <div><span>数据来源</span><strong>UCI 公开污水处理数据</strong><p>本地镜像 520 条非空记录，不是真实贵州工厂数据；TEP 保留为切换场景。</p></div>
            <div><span>模型版本</span><strong>{demoEvent.modelVersion}</strong><p>随机森林软测量预测下一条 DQO-S；历史 P95 非法规限值。</p></div>
