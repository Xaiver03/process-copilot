...
@pytest.mark.parametrize(
    ...
    [
        ...
        "三项过程变量同步变化，更支持过程偏移，而不是单点传感器故障。",
        "多个独立测量同步变化，仍需操作员复核仪表状态。",
        "三个不同测量点的过程变量在同一时间窗口内同步变化，更符合整体偏移表现；多个独立测量点的同步偏离仍需人工复核。",
        "进水流量在证据中偏离；三个不同测量点的过程变量在同一时间窗口内同步变化，更符合整体偏移。",
        "三项变量（初沉池出口 pH、进水 pH、进水流量）同时偏离各自训练中位数/IQR，且它们属于不同测点、不同参数。单点传感器故障通常只会影响单一变量，而不会让多个独立测量同步变化。因此这种同步模式更符合进水水质或水量发生过程性偏移的预期。不过事件摘要明确这些偏离仅作为检查优先级证据，不代表已证实因果，仍需操作员对仪表状态、取样时间和后续化验结果进行复核确认。",
